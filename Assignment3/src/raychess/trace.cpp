 ...
 *********************************************************************/
vec3 phong(vec3 eye, vec3 ray, vec3 surf_norm, void *obj, vec3 hit, int isplane) {
...
//
	...
	if(isplane==1) {
		struct board* pln= (struct board*)obj;
		...
		color += global_ambient * tri->mat_ambient;
		//printf("ambient: %f %f %f\n", color.x, color.y, color.z);
		//printf("mat_diffuse: %f %f %f\n", sph->mat_diffuse.x,sph->mat_diffuse.y, sph->mat_diffuse.z);
		diffuse = max_float(dot(raynormal, surf_norm),0)*light1_intensity*tri->mat_diffuse;
		//printf("diffuse: %f %f %f\n", diffuse.x, diffuse.y, diffuse.z);
		specular = pow(max_float(dot(reflect_vector, eye),0),tri->mat_shineness)*light1_intensity * tri->mat_specular;
...
//compute the shadow
vec3 get_shadow(vec3 eye, vec3 ray, vec3 surf_norm, void *obj, vec3 hit, int isplane) {
...
//	printf("not plane\n");
	struct triangle * tri = (struct triangle *)obj;
	//intermediate light
	vec3 in_normal = triangle_normal(tri, inpoint);
	float ratio;
	if(!inobj) ratio = 1.0/tri->refractivity;
	else ratio = tri->refractivity;
...
//
	...
	vec3 hit;
	int isplane;
	void *obj = intersect_scene(eye, ray, scene, &hit, &isplane);
	...
	vec3 lighthit;
	int lightisplane;
	void * light_obj = intersect_scene(hit, lightvec_normal, scene, &lighthit, &lightisplane);
	
	vec3 surf_normal = isplane?vec3(0,1,0):triangle_normal((struct triangle*)obj, hit);

	if(light_obj==NULL) {
		color += phong(-1*ray, lightvec, surf_normal, obj, hit, isplane);
	}
	else {
		if(!shadow_on) {
			color += phong(-1*ray, lightvec, surf_normal, obj, hit, isplane);
			...
			color += get_shadow(-1*ray, lightvec, surf_normal, obj, hit, isplane);
		}
	}
	...
	if(reflect_on) {
		...
		if(isplane) {
			color += ((struct board*)obj)->reflectance * recursive_ray_trace(hit, reflect_vector,num+1, inobj);
		}
		else if(!isplane)
			color += ((struct board *)obj)->reflectance * recursive_ray_trace(hit, reflect_vector, num+1, inobj);
	}
	...
	if(refract_on) {
		...
		if(refraction(hit, -1*ray, obj, isplane, inobj, &outlightvector)) {
			if(!isplane) {
		//		printf("refraction point\n");
				struct triangle * refractsph = (struct triangle *)obj;
				color += refractsph->refr*recursive_ray_trace(outlightpoint, outlightvector, num+1, !inobj);
			}
		}
	}
